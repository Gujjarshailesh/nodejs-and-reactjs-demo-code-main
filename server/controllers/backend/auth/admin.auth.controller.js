...
class AdminAuthController {
  ...
  async forgotPassword(req, res) {
    try {
      ...
      if (!isSaved) {
        throw new Error('admin.backend_forgot_password_data_not_save');
      }

      // Send an email
      await this.sendEmailOTP(user, 'telepath-forget-password');
      return response.successResponse(
        req,
        res,
  ...
  async resendOTP(req, res) {
    try {
      ...
      const isSaved = await user.save();
      if (!isSaved) {
        throw new Error('admin.auth_resend_otp_fail_data_save');
      }

      const template = 'telepath-forget-resend-password';

      await this.sendEmailOTP(user, template);
  ...
  /**
   * @name genrateRefreshToken
   * @description genrateRefreshToken
   * @param {Request} req
   * @param {Response} res
   * @returns {Json} successMessage
   */

  async genrateRefreshToken(req, res) {
    try {
      const load = req.payload.user;
      const isTokenGenerated = await common.generateRefreshToken(load);
      return response.successResponse(
        req,
        res,
